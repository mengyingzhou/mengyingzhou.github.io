...
InlineLexer.prototype.output = function(src) {
  ...
  while (src) {
    ...
    if (cap = this.rules.code.exec(src)) {
      src = src.substring(cap[0].length);
      if (cap[2][0] == ' ') cap[2] = cap[2].substr(1);
      if (cap[2][cap[2].length - 1] == ' ') cap[2] = cap[2].substr(0, cap[2].length - 1);
      out += this.renderer.codespan(escape(cap[2], true), cap[2]);
      continue;
    }
...
const GL_BibTeX = require('./bibtex-service.js');
...
Renderer.prototype.heading = function(text, level, raw) {
  ...
  if (tryId)
  {
    return '<h' + level + ' id="' + tryId[1] + '">' +
      text.substr(0, text.length - tryId[0].length).trim() +
      '</h' + level + '>\n';
  }
...
const GL_codespanSpecifiers = [
  { tests: ['tex:', 'math:', 'latex:', 'katex:'],
    before: '<!--[blog][katex]', after: '[blog]-->', escape: false },
  { tests: ['display:', 'displaymath:'],
    before: '<!--[blog][katex-display]', after: '[blog]-->', escape: false },
  { tests: ['code:'],
    before: '<code>', after: '</code>', escape: true },
  { tests: ['key:', 'kbd:'],
    before: '<kbd>', after: '</kbd>', escape: true },
  { tests: ['output:', 'out:', 'in:', 'txt:', 'text:', 'path:', 'uri:', 'url:'],
    before: '<samp>', after: '</samp>', escape: true },
  { tests: ['var:'],
    before: '<var>', after: '</var>', escape: true }
];

Renderer.prototype.codespan = function(text, raw) {
  for (var i = 0, j, x, y, z, lower = raw.toLowerCase();
    i != GL_codespanSpecifiers.length; ++i)
  {
    x = GL_codespanSpecifiers[i];
    y = x.tests;
    for (j = 0; j != y.length; ++j)
    {
      ...
      {
        raw = raw.substr(z.length);
        if (raw.endsWith(':'))
        {
          raw = raw.substr(0, raw.length - 1);
        }
        text = (x.escape ? escape(raw, true) : raw);
